Student AI Super-App — Complete Feature List
A comprehensive student platform covering exam preparation, AI-powered learning, attendance tracking, career development, student communities, events, and personal productivity.
Student-only app: students are the only login type and read-only users. No college staff login. No user uploads. All content is seeded and updated by the app maintainer via ops outside the app.

1. Dashboard & Student Profile
Personalized Student Dashboard
Student Registration and Login — Google login and email login only, no phone OTP
Password Reset and Logout All Devices
College Name and Branch Self-Declaration, No ID File Upload
Department / Branch Selection
Academic Year and Semester Selection
Subjects and Syllabus Management
Syllabus Completion Tracking — Unit-Wise Checklist and Percent-Complete
Semester Marks Entry with SGPA and CGPA Calculation and Goals
Assignment Tracker with Due Dates and Status
Student Skills and Interests
Academic Goals and Career Goals
Daily Learning Progress
Upcoming Classes and Events
Pending Tasks and Reminders
Recent Activity
Personalized Recommendations
Achievements and Milestones
Profile Editing and Management
Data Access Export with Sharing List
Grievance Request with 90-Day Response Cap
Nominee Recording and Consent Withdrawal
Age Gate with Verifiable Parental Consent for Under-18

2. Exam & Study Preparation
2.1 Question Paper Library
Previous Year Question Papers (PYQs)
Subject-Wise Question Papers
Semester-Wise Question Papers
University-Wise Question Papers
Branch-Wise Question Papers
Year-Wise Question Papers
General Competitive Exam Question Papers
Topic-Wise Questions
Chapter-Wise Questions
Question Paper Search and Filtering
Question Paper Bookmarking
Downloadable Question Papers
Question Paper PDF Viewer with Highlight, Note, and Bookmark Anchors
Maintainer-Seeded Papers, No User Uploads
Recently Viewed Papers
Organize Saved Question Papers
2.2 Study Materials
Study Material Library
Subject-Wise Study Materials
Topic-Wise Study Materials
Chapter-Wise Study Materials
Syllabus-Based Study Materials
PDF Study Material Viewer with Highlight, Note, and Bookmark Anchors
Maintainer-Seeded Materials, No User Uploads
PDF-to-Study-Material Conversion
PDF Content Summarization
Important Topics Extraction
Key Points and Definitions Extraction
Formula Extraction
Formula Rendering with Live Preview, Crisp Display, Text Fallback, and Error Preview
Important Questions Extraction
Study Material Bookmarking
Maintainer-Seeded Library, No User Uploads
Offline Study Material Access
2.3 Notes Generation
AI-Generated Notes — Defined Once Here, Referenced in Section 3
Notes for Specific Topics
Notes for Specific Chapters
Maintainer-Seeded Notes, No Upload-Based Notes
Notes from Lecture Links with Source Attribution
Short Revision Notes
Detailed Study Notes
Important Definitions
Formula-Based Notes
Formula Rendering with Live Preview, Crisp Display, Text Fallback, and Error Preview
Concept-Wise Notes
Exam-Oriented Notes
Notes Organization by Subject
Notes Organization by Chapter
Edit and Save Generated Notes
Export Notes
Search Saved Notes
2.4 Test and Mock Exam System
AI Test Generator — Defined Once Here, Referenced in Section 3
AI Mock-Test Generator — Defined Once Here, Referenced in Section 3
Practice Tests
Subject-Wise Tests
Topic-Wise Tests
Chapter-Wise Tests
Syllabus-Based Tests
Previous Year Question-Based Tests
PDF-to-Question Generation
PDF-to-Test Generation
Multiple-Choice Questions
True/False Questions
Short-Answer Questions
Long-Answer Questions
Difficulty-Level Selection
Custom Test Creation
Timed Examinations
Exam Simulation Mode
Automatic Answer Evaluation for Supported Question Types
Test Results and Scores
Answer Explanations
Review Incorrect Answers
Test History
Retake Tests
Performance Analysis
2.5 General Exam Preparation
General Exam Preparation
Competitive Exam Preparation Resources
General Knowledge Practice
Current Affairs Preparation
Aptitude Practice
Logical Reasoning Practice
Verbal Ability Practice
Technical Question Practice
Topic-Wise Practice
Exam Preparation Progress Tracking

3. AI Learning & Study Assistant
AI Study Assistant
AI Doubt-Clearing Assistant
Topic Explanation
Chapter Explanation
Step-by-Step Concept Explanation
Simplified Explanations for Difficult Topics
Examples-Based Learning
Follow-Up Questions
Personalized Learning Recommendations
AI Question Generation
AI Study-Material Generation
AI Notes Generation — See Section 2.3 Single Definition
AI Test Generation — See Section 2.4 Single Definition
AI Mock-Test Generation — See Section 2.4 Single Definition
PDF Content Understanding
PDF Question Answering
Important Topic Identification
Weak Topic Identification
Revision Recommendations
Study Progress Analysis

4. Flashcards & Spaced Repetition — REMOVED v0.15.0 to control AI and storage cost. Use notes plus tests for revision. No decks, no Anki scheduling, no due notifications.

5. Timetable & Student Attendance Tracking
5.1 Timetable Management
Weekly Timetable Setup — Enter or upload the regular weekly timetable.
Day-Wise Timetable — View classes scheduled for each day.
Period-Wise Timetable — Display individual periods with their subjects and timings.
Subject and Classroom Details — Store subject names, faculty display names, and classroom information. Faculty names are display-only, no staff login.
Semester-Based Timetable — Maintain a timetable for the current semester.
Timetable Editing — Add, edit, or remove regular timetable periods.
Timetable Updates — Update the recurring timetable when the semester schedule changes.
5.2 One-Day Timetable Changes
Date-Specific Timetable Editing — Change the schedule for one selected date without affecting the regular weekly timetable.
One-Day Subject Changes — Replace a scheduled subject for a particular day.
One-Day Period Changes — Change a period's timing or position for a particular date.
Extra Classes — Add classes scheduled outside the regular timetable.
Cancelled Classes — Mark classes that will not take place.
Rescheduled Classes — Move a class to another period or time for a specific day.
Classroom Changes — Update the classroom for a particular class.
Holiday Management — Mark holidays when regular classes do not take place.
Restore Regular Timetable — Return to the recurring schedule after a date-specific exception.
5.3 Attendance Management
Self-Marked Attendance Only — No Faculty Verification, No College Staff Login
Quick Attendance Marking — Mark each class as Present, Absent, or Unmarked.
Period-Wise Attendance — Record attendance for individual periods.
Subject-Wise Attendance — Calculate attendance separately for each subject.
Overall Attendance Percentage — Display the student's overall attendance.
Attendance History — View attendance records by date, period, and subject.
Attendance Target Setting — Set a desired attendance percentage.
Low-Attendance Alerts — Highlight subjects below the target.
Attendance Recovery Estimate — Estimate how many future classes must be attended to reach the target.
Automatic Attendance Recalculation — Update attendance when classes or attendance records change.
Attendance Dashboard — Show attendance totals, percentages, missed classes, and upcoming classes.
Important rule: Cancelled classes and holidays should not count as absences. Date-specific timetable changes should affect only the selected date unless the student explicitly updates the regular timetable.

6. Career Development & Career Roadmap
Personalized Career Roadmap
Career Path Explorer
Career Role Information
Career Specialization Explorer
Step-by-Step Learning Path
Skill Gap Analysis
Skill Development Plan
Role-Based Skill Requirements
Career Progress Tracking
Career Milestones
Short-Term Career Goals
Long-Term Career Goals
Career Readiness Assessment
Skill-Based Recommendations
Certification Recommendations
Project Recommendations
Internship Preparation Guidance
Placement Preparation Guidance
Technical Interview Preparation
Mock Interview Practice
Resume Guidance
Portfolio Development Guidance
Career Roadmap Reminders
Roadmap Adjustment
Career Progress Dashboard
Monthly Career Highlights
Career Opportunity Discovery

7. Events & Opportunities
7.1 Events Discovery
Upcoming Workshops
Upcoming Hackathons
Competitions and Technical Events
Seminars and Conferences
College Events
Online Events
Offline Events
Technology-Based Event Filtering
Domain-Based Event Filtering
Location-Based Event Filtering
Event Search
Event Details
Event Eligibility Information
Registration Deadlines
Event Reminders
Bookmark and Save Events
Registered Events Tracking
Upcoming Event Dashboard
7.2 Opportunities
Internship Discovery
Scholarship Discovery
Student Program Discovery
Research Opportunities
Project Opportunities
Certification Opportunities
Career-Related Opportunities
Opportunity Eligibility Details
Application Deadline Tracking
Saved Opportunities
Opportunity Notifications

8. Student Team-Up & Community
8.1 Find Teammates
Same-College Team-Up
Find Students from the Same College
Find Teammates by Skills
Find Teammates by Branch
Find Teammates by Academic Year
Find Teammates by Interests
Find Teammates by Project Requirements
Find Teammates for Hackathons
Find Teammates for Workshops
Find Teammates for Competitions
Looking-for-Teammates Posts
Team Invitations
Student Profile Discovery
8.2 Team Management
Create Project Teams
Join Existing Teams
Hackathon Team Formation
Workshop Team Formation
Competition Team Formation
Team Member Management
Team Roles and Responsibilities
Team Chat
Project Collaboration
Team Activity Updates
Team Invitations and Requests
Team Project Workspace
8.3 Student Communities
College Communities
Department Communities
Branch-Wise Communities
Subject-Wise Communities
Career-Based Communities
Project-Based Communities
Discussion Posts
Questions and Answers
Community Announcements
Community Event Sharing
Report Post and User, Block User, Spam Filter, Auto-Hide and Auto-Takedown with Audit Trail

9. Project Development & Hackathon Support
Project Idea Discovery
Project Team Creation
Project Workspace
Project Description and Goals
Task Assignment
Task Progress Tracking
Team Collaboration
Project Discussion
Project Resource Sharing
Project Milestone Tracking
Hackathon Preparation Checklist
Hackathon Team Management
Hackathon Task Tracking
Project Documentation Support
Project Portfolio Entries
Project Completion Tracking
Project Showcase

10. Current Affairs, GK & Seminars
Current Affairs
Today’s GK
Daily General Knowledge Questions
GK Practice Tests
Current Affairs Quizzes
Topic-Wise GK
Technology News and Updates
Seminar Resources
Seminar Information
Seminar Learning Materials
Seminar Notes
Seminar Reminders
Saved GK and Current Affairs Resources

11. Fitness & Student Wellness
Health Planner and Student Diet Planner are excluded. General wellness reminders stay in scope.
Fitness Tracking
Daily Activity Tracking
Workout Activity Logging
Fitness Progress Overview
Study-Break Reminders
Hydration Reminders
Study and Rest Balance Reminders
Health Alerts
Health/Wellness Mode
Personalized Wellness Recommendations
Wellness Reminder Settings

12. Notifications & Reminders
Class Reminders
Attendance Reminders
Low-Attendance Alerts
Study Reminders
Flashcard Review Reminders
Test and Mock Exam Reminders
Assignment and Task Reminders
Workshop Reminders
Hackathon Reminders
Competition Reminders
Internship Deadline Reminders
Career Milestone Reminders
Team Invitations
Team Activity Notifications
Community Notifications
Custom Notification Preferences

13. Search, Analytics & Progress Tracking
Global App Search
Search Question Papers
Search Study Materials
Search Notes
Search Flashcards
Search Events and Opportunities
Search Students and Teams
Academic Progress Dashboard
Study Time Tracking
Subject-Wise Progress
Test Performance Analytics
Topic-Wise Performance
Weakness Detection
Targeted Practice Recommendations
Retest Recommendations
Flashcard Review Statistics
Career Roadmap Progress
Skill Development Progress
Attendance Analytics
Achievement Tracking

14. Settings, Privacy & Account Management
Account Settings with Nominee Recording
Profile Management
Notification Preferences
Privacy Settings with Data Access Export and Sharing List
Grievance Redressal Contact with 90-Day Response Cap
Consent Withdrawal and Age Gate with Verifiable Parental Consent for Under-18
Student Profile Visibility Controls
Community Participation Controls
Team Invitation Preferences
Saved Content Management
Offline Content Management
Data and Account Management with Erasure at Purpose End
Help and Support
Feedback and Issue Reporting

Core Student Journey
Learn → Practice → Test → Identify Weaknesses → Improve → Track Attendance → Develop Skills → Build Projects → Find Teammates → Discover Opportunities → Prepare for a Career
This feature list combines the app's major modules, including the expanded career roadmap and the combined weekly/date-specific timetable and attendance system.
Vision update 2026-09-21 v0.20.0: agent-ready freeze, no scope change beyond v0.19.0 read-only. Prior v0.19.0: read-only users, maintainer seeds all content, no uploads including ID files. Prior v0.15.0: flashcards Sec 4 removed for AI and storage cost. Prior v0.10.0: formula rendering lines added. Prior v0.9.0: student-only scope, P0 academic and DPDP rights added, AI single-definition refs, moderation and diet and attendance rules clarified. Backend and limits live in SUPABASE-PLAN.md and ROADMAP.md, not duplicated here.
